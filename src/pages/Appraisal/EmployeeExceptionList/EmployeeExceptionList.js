...
const EmployeeExceptionList = () => {
  ...
  const roleName = getUserProperty(
    'roleName',
    employeeDetails?.currentUser?.[0]?.ROLE_NAME ||
      employeeDetails?.currentUser?.[0]?.roleName ||
      ''
  );
  const roleId = getUserProperty(
    'roleId',
    employeeDetails?.currentUser?.[0]?.ROLE_ID || employeeDetails?.currentUser?.[0]?.roleId || ''
  );
  const zone = getUserProperty(
    'zone',
    ...
    if (apiData) {
      ...
      if (responseData?.result && Array.isArray(responseData.result)) {
        const mappedData = responseData.result.map((item) => ({
          ...
          primaryRole:
            item.primary || item.primary_role || item.PRIMARY_ROLE || item.role || item.ROLE || '',
          branch:
            item.BRNAME || item.branch || item.BRANCH || item.branch_name || item.BRANCH_NAME || '',
          preScore: `${item.TOTAL_SCORE || ''}/${item.TOTAL_MAX_SCORE || ''}`,
          postScore: `${item.TOTAL_FINAL_SCORE || ''}/${item.TOTAL_MAX_SCORE || ''}`,
          status:
    ...
    <div className="pageWrapper">
      ...
      <div className="filter-section d-flex flex-wrap align-items-end gap-3 mb-3">
        ...
        <div className="d-flex align-items-end gap-2">
          <button className="btn search-btn px-4" onClick={handleSearch}>
            Search
          </button>

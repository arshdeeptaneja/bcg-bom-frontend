...
export const useExceptionValidator = ({
  ...
  useEffect(() => {
    ...
    const sourceRows =
      reviewData?.results_KRA_LIST_Measurable ||
      reviewData?.result?.kraData ||
      reviewData?.result ||
      [];
  ...
  const scoreData = useMemo(
    () => ({
      ...
      maxScoreTotal: reviewData?.measurable_maxscore_total || 0,
      hasScores: Boolean(
  ...
  const handleDownload = useCallback(() => {
    const attachmentUrl =
      reviewData?.file_url || reviewData?.attachmentUrl || reviewData?.result?.attachmentUrl;
    if (!attachmentUrl) {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (response) => {
      const successMessage =
        response?.RESPONSE === 'validator_exception_approved'
